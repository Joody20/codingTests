...
bridge = [0] * w  # 다리의 길이 -> 즉, 트럭의 무게를 저장하는거임.
time = 0 # 다리를 건너는 최단시간
total = 0  # 트럭의 총 무게

while(bridge): # 다리에 트럭이 다 지나갈 때 까지
    time += 1  # 시간을 늘려주는거야. 아 다리의 길이만큼씩....
    total -= bridge.pop(0)  # 다리에 가장 앞에 있는 트럭을 내보내주는거래. total에 더해지지 않게.

    if trucks:  # 아직 대기 중인 트럭이 있다면, 
        if total + trucks[0] <= l:  # 다리 위에 있는 트럭의 무게와 trucks의 첫번째꺼 더한게 l보다 작으면
            total += trucks[0] # total에 새 truck의 무게를 더해주고
            bridge.append(trucks.pop(0))  # 다리 맨 끝 칸에 들어갈 트럭의 무게를 넣어주는거래.

        else:  # l보다 크면
            bridge.append(0)  # 그냥 맨 끝칸에는 0을 넣어주는거지.
