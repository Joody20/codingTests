...
# 매우큰 점프를 사용한 dp와 사용하지 않은 dp로 나누어서 구해주래.
dp = [float("inf")] * N
dp_k = [float("inf")] * N
...
#매우큰 점프 사용하지 않았을 때 dp 구하기
for i in range(N-1):
    #작은 점프
    if i + 1 < N:
        dp[i+1] = min(dp[i+1], dp[i] + small[i])

    #큰 점프
    if i + 2 < N:
        dp[i+2] = min(dp[i+2], dp[i] + big[i])


# 매우 큰 점프를 사용했을 때 dp구하기
dp_k[0] = 0

for i in range(N):
    # 매우 큰 점프
    if i + 3 < N:
        dp_k[i+3] = min(dp_k[i+3], dp[i] + K)

    # 기존에 쓴 점프에서 작은 점프
    if i + 1 < N:
        dp_k[i+1] = min(dp_k[i+1] , dp_k[i] + small[i])

    # 기존에 쓴 점프에서 큰 점프
    if i + 2 < N:
        dp_k[i+2] = min(dp_k[i+2] , dp_k[i] + big[i])


print(min(dp[N-1] , dp_k[N-1]))
