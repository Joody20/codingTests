...
for _ in range(T):
    K = int(input())  # 장의 수
    cards =list(map(int, input().split()))  # 파일의 크기

    heapq.heapify(cards)  # 우선순위 큐 형태로 바꿔주고
    ...
    if len(cards) == 1:
        ...
        while cards:
            if len(cards) == 2:  # 이게 마지막 결과를 리턴하는거네 -> 마지막엔 두개의 숫자만 남겟네
                print(total + sum(cards))
                break

            a = heapq.heappop(cards) # 이렇게 빼내니까
