...
while True:
    ...
    players = sum([list(map(int,input().split())) for _ in range(N)],[])

    counts = Counter(players)
    second = sorted(counts.values(), reverse=True)[1]  # 두번째로 높은 랭킹 소유 점수

    for k,v in counts.items():
        if v == second:
            result.append(k)

    sorting_res = sorted(result)

    print(' '.join(map(str,sorting_res)))
