"""
백준 골드 5: https://www.acmicpc.net/problem/20207
...
N = int(input()) # 일정의 개수
arr = [0] * 366  # 일정의 개수를 넣는 배열 365일이니까 366개의 배열을 만들어주는거야.
ans = 0  # 코팅지의 면적

for _ in range(N):  # 일정의 개수 만큼
    start ,end = map(int,input().split())  # 시작날짜와 종료 날짜를 빼

    while start <= end:  # 시작날짜가 종료날짜보다 작거나 같을 때 까지
        arr[start] += 1  # 시작날짜를 arr에 넣어줘. 그리고 1 더해줌.
        start += 1
...
for i in range(366):
    if arr[i] == 0:    # arr[i]가 0이면
        ans += rows * cols  # ans += rows*cols  24 , 4 가 나오겟지
        rows, cols = 0,0  # 계산한건 0,0으로 다시 바꿔줘
        continue
    rows += 1  # rows를 하나씩 늘리고
    cols = max(cols,arr[i])  # cols도 최대값으로 해주고ㅡ

# print(arr)
print(ans + rows*cols)  # 마지막에 계산해주는거야  ans += rows * cols