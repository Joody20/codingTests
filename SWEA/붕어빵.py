...
for t in range(1,T+1):
    ...
    for i in range(N):
        time = arrive[i]  # 손님 도착 시간
        bread = (time // M) *  K  # 그 시간안에 만들 수 있는 빵의 개수 

        if (i+1) > bread:  # 빵의 개수보다 손님이 더 많아
            possible = False # 그럼 불가능하겠지?
            break

    result = "Possible" if possible else "Impossible"  # 그걸 이제 리턴하기 위함
